...
function findByPrefix(prefix) {
    return courses.filter(courses => courses.prefix.indexOf(prefix) > -1);
}

//To do: implement updateById(id, course)
function updateById(id, course) {
    if (courses.indexOf(findById(id)) == -1){
        return false;
    } else {
        courses.splice(courses.indexOf(findById(id)), 1, course);
        return true;
    }
}
...
function removeById(id) {
    if (courses.indexOf(findById(id)) == -1){
        ...
        courses.splice(courses.indexOf(findById(id)), 1);
        return true;
    }
}
